...
The plugin owns MCP via `.mcp.json`. Do not also add `[mcp_servers.chatgpt-oauth]` to `config.toml`. `[model.luna]` cannot ship in a plugin; that block stays in user config.
