...
description: >
  ...
  mentions luna, ChatGPT OAuth, or a Codex child.
  Not a platform API key. Not the child agent (that type is openai-auth).
when-to-use: spawn openai-auth, ChatGPT OAuth child, gpt-5.6-luna, luna, loopback health, codex login
...
3. **Route to luna.** Catalog slug `luna` sends `model=gpt-5.6-luna` through `:8743`. If `openai-auth` is not in this TUI's type enum, spawn `general-purpose` with `model=luna`.
4. **Trust the child's self-report.** It must say `gpt-5.6-luna`. If it says `grok-4.x`, the live session is missing `[model.luna]` or started before that block existed.
